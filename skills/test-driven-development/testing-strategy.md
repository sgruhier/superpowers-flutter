...
This document outlines our shared testing approach for a Ruby on Rails application using flutter_test. It is written for senior Ruby developers and intended to stay flexible. Use it when writing tests manually or collaborating with AI-generated tests. Its purpose is to support clear, consistent decisions across the team.
...
> for HTML assertions. Never use Capybara or `flutter test:system`.
...
- **flutter_test**: the default Rails test framework. Use `test "description" do ... end` syntax throughout.
...
## 3.2 Built-in flutter_test Assertions
...
```bash
flutter test test/models/project_test.rb       # single file
flutter test test/models/project_test.rb:42    # specific line
flutter test --fail-fast                        # stop on first failure
flutter test                                    # full suite (no system tests)
flutter test:system                             # system tests only
flutter test:all                                # everything including system tests
```
...
```bash
PARALLEL_WORKERS=4 flutter test
```
...
- name: Setup Database
  run: |
    flutter db:create RAILS_ENV=test
    flutter db:schema:load RAILS_ENV=test
```
...
```bash
flutter test:db   # rebuilds the test database from schema
```
...
```bash
flutter test --seed 12345
PARALLEL_WORKERS=4 flutter test --seed 12345
```
...
- Profile slow tests with `flutter test --profile` before optimising.
