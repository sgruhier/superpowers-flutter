...
Run `superpowers-flutter:compound` after solving problems to start building your knowledge base.
```
...
- **Update territory** — file paths moved, classes renamed, links broke, metadata drifted, but the core recommended approach is still how the code works. `superpowers-flutter:compound-refresh` fixes these directly.
- **Replace territory** — the recommended solution conflicts with current code, the architectural approach changed, or the pattern is no longer the preferred way. This means a new learning needs to be written. A replacement subagent writes the successor following `superpowers-flutter:compound`'s document format (frontmatter, problem, root cause, solution, prevention), using the investigation evidence already gathered. The orchestrator does not rewrite learnings inline — it delegates to a subagent for context isolation.
...
- **Insufficient evidence** — the drift is so fundamental that you cannot confidently document the current approach. The entire subsystem was replaced, or the new architecture is too complex to understand from a file scan alone. → Mark as stale in place:
   ...
   - Recommend the user run `superpowers-flutter:compound` after their next encounter with that area, when they have fresh problem-solving context
...
- You are about to create a successor via `superpowers-flutter:compound`
...
2. The subagent writes the new learning following `superpowers-flutter:compound`'s document format: YAML frontmatter (title, category, date, module, component, tags), problem description, root cause, current solution with code examples, and prevention tips. It should use dedicated file search and read tools if it needs additional context beyond what was passed.
...
3. Recommend the user run `superpowers-flutter:compound` after their next encounter with that area
...
- `superpowers-flutter:compound` captures a newly solved, verified problem
- `superpowers-flutter:compound-refresh` maintains older learnings as the codebase evolves

Use **Replace** only when the refresh process has enough real evidence to write a trustworthy successor. When evidence is insufficient, mark as stale and recommend `superpowers-flutter:compound` for when the user next encounters that problem area.
